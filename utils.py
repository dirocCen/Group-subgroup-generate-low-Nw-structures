...
from sagar.io.vasp import _write_string, read_vasp
...
root = "/home/cenyj/study_document/pycode/sagarWeb-master"
...
def devide_inequitable_layers(surf, precision=5):
    pos = surf.get_scaled_positions()
    pos = np.round(pos, precision)
    z_axis = np.unique(pos[:, 2])
    print(z_axis)
    set_trace()
...
def inequivalent_atom(poscar, ele=None, sym=1e-3):
    if ele == None:
        cell = read_vasp(poscar)
        # set_trace()
        atom_uniq_type = len(np.unique(cell.get_symmetry(sym)['equivalent_atoms']))
    else:
        cell = read_vasp(poscar)
        idx = np.where(cell.atoms==ele)[0]
        if len(idx)==0:
            atom_uniq_type = 0
        else:
            atom_uniq_type = len(np.unique(cell.get_symmetry(sym)['equivalent_atoms'][idx]))
        # set_trace()
        # cell.get_symmetry(sym)['equivalent_atoms']
    return atom_uniq_type


def get_ele_num(pos,ele):
    cell = read_vasp(pos)
    num = len(np.where(cell.atoms==ele)[0])
    return num
